...
* **Email-based OTP Verification System:** Replaced SMS verification with a 100% free **Email OTP Verification System** using serverless endpoints ([`src/app/api/email/send-otp/route.ts`](file:///C:/Users/DELL/.gemini/antigravity/scratch/eshwar-home-needs/src/app/api/email/send-otp/route.ts) and [`src/app/api/email/verify-otp/route.ts`](file:///C:/Users/DELL/.gemini/antigravity/scratch/eshwar-home-needs/src/app/api/email/verify-otp/route.ts)). When triggered, Nodemailer dispatches a real HTML email containing a 6-digit verification code to the customer's email address (delivering a push notification straight to their mobile device). If SMTP variables are not set in `.env.local`, the server console automatically logs the OTP code for local mock testing.
* **Buy/Sell Verification Priority Gates:** Extensively gated B2C buying and B2B/Scrap selling operations. Unverified users are blocked from placing orders, adding products to their cart, booking doorstep scrap collection pickups (`src/app/scrap/page.tsx`), and submitting B2B wholesale quotation requests (`src/app/wholesale/page.tsx`).
* **Profile Phone Number Editing:** Added inline **"Edit"** and **"Add Phone Number"** form fields in the customer Account profile tab. To prevent verification spoofing, updating or registering a new phone number automatically resets `phoneVerified` to `false` in Firestore, requiring a fresh OTP check.
