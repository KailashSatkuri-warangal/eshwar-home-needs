...
* **Real Firebase Phone OTP Authentication:** Replaced the simulated modal with standard client-side **Firebase Phone Authentication** (`src/components/ui/OtpVerificationModal.tsx`). It runs an invisible Recaptcha Verifier and prompts Firebase to dispatch a **real SMS verification text message** directly to the customer's physical mobile device. Once verified, the confirmation token saves successfully to Firestore.
